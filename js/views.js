...
window.Views = (function () {
      ...
      hero =
        '<div class="hero">' +
          '<div>' +
            ...
            '<h1>Module ' + volgende.module.nr + ' · Les ' + E(volgende.les.nr) + ' - ' + E(volgende.les.titel) + '</h1>' +
            '<p>' + E(volgende.les.leerdoel || volgende.module.intro) + '</p>' +
            '<div class="acts">' +
              '<a class="btn" href="#/les/' + volgende.les.id + '">▶ ' + (isStart ? 'Start de cursus' : 'Verder leren') + '</a>' +
              '<a class="btn ghost" href="#/module/' + volgende.module.id + '">Moduleoverzicht</a>' +
            '</div>' +
          '</div>' +
          '<div class="ring" style="--p:' + st.procent + '"><div><b>' + st.procent + '%</b><span>Voltooid</span></div></div>' +
        '</div>';
    } else {
      hero = '<div class="hero"><div><h1>Cursus geladen</h1><p>Nog geen lessen gevonden.</p></div></div>';
    }
  ...
  function moduleKaart(m) {
    ...
    return '<a class="modcard" href="#/module/' + m.id + '">' +
  ...
  function modulePagina(id) {
    ...
    return '<div class="crumbs"><a href="#/">Mijn cursus</a> › Module ' + m.nr + '</div>' +
  ...
  function lesRij(m, l) {
    ...
    return '<a class="lrow ' + (klaar ? 'done' : isNu ? 'now' : '') + '" href="#/les/' + l.id + '">' +
  ...
  function lesPagina(id) {
    ...
    var artikel = '<article class="article">' +
      '<div class="crumbs"><a href="#/">Mijn cursus</a> › <a href="#/module/' + m.id + '">Module ' + m.nr + '</a> › Les ' + E(l.nr) + '</div>' +
      '<h1>' + E(l.titel) + '</h1>' +
      ...
      checklist +
      '<div class="footnav">' +
        (buren.vorige ? '<a class="btn ghost sm" href="#/les/' + buren.vorige.les.id + '">← ' + E(buren.vorige.les.nr) + ' ' + E(buren.vorige.les.titel) + '</a>' : '<span></span>') +
        '<div class="right">' +
          '<button class="btn' + (klaar ? ' done-state' : '') + '" id="btnKlaar">' +
            (klaar ? '✓ Afgerond' : '✓ Markeer als afgerond') + '</button>' +
          (buren.volgende ? '<a class="btn ghost sm" href="#/les/' + buren.volgende.les.id + '">' + E(buren.volgende.les.nr) + ' ' + E(buren.volgende.les.titel) + ' →</a>' : '') +
        '</div>' +
      '</div>' +
  ...
  function naslag() {
    return '<div class="crumbs"><a href="#/">Mijn cursus</a> › Naslag</div>' +
      '<div class="sect-h"><h2>Naslag</h2><span>de dingen die je op de werkvloer wilt opzoeken</span></div>' +
